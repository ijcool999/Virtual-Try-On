...
def bodyDetection(frame, face_cascade, filename):
    ...
    for (x,y,w,h) in face:
        ...
        img_bg = cv.bitwise_and(roi_color,roi_color,mask = mask_inv)
        ...
        img_fg = cv.add(img_bg,img_fg)
           # Resize the image to fit on upper body size
        img_fg = cv.resize(img_fg,(width,height))
            # Take the upper body frame region and combine with the img_fg
        added_image =  cv.addWeighted(frame[ 0 :height, 0:width,:],alpha,img_fg[0 :height, 0: width,:],1-alpha,0)
